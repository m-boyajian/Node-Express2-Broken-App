...
require('fast-text-encoding').TextDecoder;
...
describe("POST /auth/register", function() {
  ...
  test("should have only one module.exports = app;", () => {
    expect.assertions(2);
    const keys = Object.keys(app);
    expect(keys).toHaveLength(1);
    expect(keys[0]).toEqual("listen");
  });
});
...
describe('GET /users - Authentication', () => {
  ...
  test("should list all users when authenticated", async () => {
    const response = await request(app)
      .get("/users")
...
describe('GET /users - User Information', () => {
  test('should return only basic user information', async () => {
    const response = await request(app).get('/users');
...
describe("GET /users/[username]", function() {
  ...
  test("should throw error when user is not found", async () => {
    expect.assertions(2);
    try {
      await request(app).get("/users/nonexistent_user").send({ _token: tokens.u1 });
    } catch (error) {
      expect(error.response.statusCode).toBe(404);
      expect(error.response.body).toEqual({ status: 404, message: "No such user" });
    }
  });
});
...
describe("PATCH /users/[username]", function() {
  ...
  test("should disallow updating not-allowed fields", async function() {
    // Attempt to update user with unauthorized fields
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, admin: true });
    expect(response.statusCode).toBe(401);
...
describe("DELETE /users/[username]", function() {
  ...
  test("should return 404 if user to delete not found", async function() {
