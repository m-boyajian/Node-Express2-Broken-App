...
/** Register user with data. Returns new user data. */
  ...
  static async update(username, data) {
    // Change fields to only allow f-name, l-name, phone & email
    const allowedFields = ['first_name', 'last_name', 'phone', 'email'];

    const filteredOutData = Object.keys(data)
...
;      }, {});
  ...
  static async delete(username, isAdmin) {
    if (!isAdmin) {
      throw new ExpressError('Only allowed by admin', 401);
    }
    const result = await db.query(
